...
Ogre::Mesh *CreateBox(Ogre::SceneManager *scene, Ogre::String name, double w, double h, double d, Ogre::ColourValue color);
...
GameWorld::GameWorld(Game& game, std::string sceneName) : m_game(game), m_scene(nullptr)
{
	...
	// create some cars
	for (int i = 0; i < 10; ++i) {
		m_objects.push_back(GameObject("CarBox", 30, 80, 20));
	...
	for (int i = 0; i < 10; ++i) {
		m_objects.push_back(GameObject("PedBox", 180, 2, 2));
...
Ogre::Mesh *CreateBox(Ogre::SceneManager *scene, Ogre::String name, double w, double h, double d, Ogre::ColourValue color)
{
	...
	temp->end();
	return temp->convertToMesh(name).get();
}
